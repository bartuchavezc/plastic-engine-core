...
func TestDamerauLevenshteinDistance(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.a+"→"+tt.b, func(t *testing.T) {
			got := indexstore.DamerauLevenshteinDistance(tt.a, tt.b)
			if got != tt.want {
...
func TestFuzzyPrefixes(t *testing.T) {
	...
	prefixes := indexstore.FuzzyPrefixes("heaslight", 1)
	if len(prefixes) == 0 {
...
func TestManagerListTermsByFuzzy(t *testing.T) {
	...
	docs := []indexstore.DocumentBatch{
		{DocID: "doc-1", FieldTerms: map[string][]indexstore.TermPosting{
			"title": {{Term: "headlight", TF: 1, Positions: []int{0}}},
		}},
		{DocID: "doc-2", FieldTerms: map[string][]indexstore.TermPosting{
			"title": {{Term: "headlight", TF: 1, Positions: []int{0}}},
		}},
		{DocID: "doc-3", FieldTerms: map[string][]indexstore.TermPosting{
			"title": {{Term: "headline", TF: 1, Positions: []int{0}}},
		}},
		{DocID: "doc-4", FieldTerms: map[string][]indexstore.TermPosting{
