...
func ComputeShardKey(def index.IndexDefinition, documentID string, payload json.RawMessage, routing RoutingMetadata) (string, error) {
	switch def.ShardStrategy {
	case index.ShardStrategyAutomatic:
		return computeAutomaticKey(def, documentID, payload, routing)
	case index.ShardStrategyDate:
		return computeDateKey(def, payload, routing)
	case index.ShardStrategyComputed:
...
func computeAutomaticKey(def index.IndexDefinition, documentID string, payload json.RawMessage, routing RoutingMetadata) (string, error) {
...
func computeDateKey(def index.IndexDefinition, payload json.RawMessage, routing RoutingMetadata) (string, error) {
...
func computeComputedKey(def index.IndexDefinition, payload json.RawMessage, routing RoutingMetadata) (string, error) {
...
func transformComponentValue(value string, transform index.ComputedTransform) string {
	switch transform {
	case index.ComputedTransformExact:
		return sanitizeKey(value)
	case index.ComputedTransformSlug:
		return sanitizeKey(value)
	default:
		return sanitizeKey(value)
	}
