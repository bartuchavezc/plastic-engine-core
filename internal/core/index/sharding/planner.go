...
import (
	...
	"plastic-engine-core/internal/core/index"
)

// Planner decides which shards must be materialised for an index definition.
type Planner interface {
	PlanInitialShards(ctx context.Context, def index.IndexDefinition, seeds []string) ([]ShardSpec, error)
}
...
type PlannerFactory struct {
	planners map[index.ShardStrategy]Planner
}
...
func NewFactory() *PlannerFactory {
	return &PlannerFactory{
		planners: map[index.ShardStrategy]Planner{
			index.ShardStrategyAutomatic: automaticPlanner{},
			index.ShardStrategyDate:      datePlanner{},
			index.ShardStrategyComputed:  computedPlanner{},
		},
	}
}

// Get returns the planner associated with the strategy.
func (f *PlannerFactory) Get(strategy index.ShardStrategy) (Planner, error) {
...
func (automaticPlanner) PlanInitialShards(_ context.Context, def index.IndexDefinition, _ []string) ([]ShardSpec, error) {
	count := 1
...
func (datePlanner) PlanInitialShards(_ context.Context, def index.IndexDefinition, seeds []string) ([]ShardSpec, error) {
...
func (computedPlanner) PlanInitialShards(_ context.Context, _ index.IndexDefinition, seeds []string) ([]ShardSpec, error) {
	if len(seeds) == 0 {
...
func formatDateKey(t time.Time, granularity index.DateGranularity) string {
	switch granularity {
	case index.DateGranularityYear:
		return fmt.Sprintf("%04d", t.Year())
	case index.DateGranularityDay:
