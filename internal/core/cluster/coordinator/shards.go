...
func (c *Coordinator) planInitialShards(ctx context.Context, def coreindex.IndexDefinition) error {
	if c == nil || c.db == nil {
	...
	c.Logger().Info("planned initial shards",
		helpers.Field{Key: "index_id", Value: def.ID},
		helpers.Field{Key: "shards", Value: len(specs)},
	)

	return nil
}

func (c *Coordinator) assignShardsToNode(ctx context.Context, nodeID string, limit int) ([]searchshard.Assignment, error) {
	if c == nil || c.db == nil {
...
func assignShardsToNodeTx(ctx context.Context, tx *sql.Tx, nodeID string, limit int) ([]searchshard.Assignment, error) {
	return shardspkg.AssignShardsToNodeTx(ctx, tx, nodeID, limit)
}

func loadAssignmentsForNodeTx(ctx context.Context, tx *sql.Tx, nodeID string) ([]searchshard.Assignment, error) {
	...
	for _, node := range nodes {
		...
		if len(assignments) > 0 {
			c.Logger().Info("assigned shards to node",
				helpers.Field{Key: "node_id", Value: node.ID},
				helpers.Field{Key: "count", Value: len(assignments)},
			)
		}
	}
