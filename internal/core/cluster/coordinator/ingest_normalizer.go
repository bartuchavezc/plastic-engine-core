...
func normalizeDocumentPayload(def coreindex.IndexDefinition, raw json.RawMessage) (json.RawMessage, error) {
	...
	if len(trimmed) == 0 {
		...
		if err := json.Unmarshal(trimmed, &doc); err != nil {
			return nil, coreindex.NewValidationError("payload must be a JSON object")
		}
		if doc == nil {
			doc = make(map[string]any)
		}
	...
	for _, field := range def.FieldMappings {
		...
		if !exists || isMissingValue(value) {
			if field.Required {
				return nil, coreindex.NewValidationError(fmt.Sprintf("field %q is required", field.Name))
			}
			continue
		}
...
func normalizeFieldValue(field coreindex.FieldMapping, value any) (any, error) {
	switch field.Type {
	case coreindex.FieldTypeText, coreindex.FieldTypeKeyword:
		str, ok := value.(string)
		if !ok {
			return nil, coreindex.NewValidationError(fmt.Sprintf("field %q must be a string", field.Name))
		}
		if field.Required && strings.TrimSpace(str) == "" {
			return nil, coreindex.NewValidationError(fmt.Sprintf("field %q cannot be empty", field.Name))
		}
		return str, nil

	case coreindex.FieldTypeInteger:
...
func normalizeIntegerValue(fieldName string, value any) (int64, error) {
	...
	case float64:
		if math.Trunc(v) != v {
			return 0, coreindex.NewValidationError(fmt.Sprintf("field %q must be an integer", fieldName))
		}
		return int64(v), nil
	case string:
...
func normalizeDateValue(fieldName string, value any) (string, error) {
	...
	if err != nil {
		return "", coreindex.NewValidationError(fmt.Sprintf("field %q must be a valid date", fieldName))
	}
