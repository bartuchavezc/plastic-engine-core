...
type Coordinator struct {
	...
	indexRepo           *coreindex.Repository
...
func NewCoordinator(role string, port string, dbPath string) (*Coordinator, error) {
	...
	repo := coreindex.NewRepository(db)
...
// Close releases database resources held by the coordinator.
...
func (c *Coordinator) Logger() helpers.Logger {
	return c.log
}
...
func (c *Coordinator) CreateIndex(ctx context.Context, req coreindex.CreateIndexRequest) (coreindex.CreateIndexResponse, error) {
	ctx, span := otel.Tracer("coordinator.index").Start(ctx, "Coordinator.CreateIndex")
	defer span.End()

	if c == nil || c.indexRepo == nil {
		return coreindex.CreateIndexResponse{}, errors.New("coordinator not initialised")
	}

	resp, err := c.indexRepo.CreateIndex(ctx, req)
	if err != nil {
		c.log.Error("create index failed", helpers.Field{Key: "error", Value: err}, helpers.Field{Key: "index_id", Value: req.ID})
		return coreindex.CreateIndexResponse{}, err
	}

	c.log.Info("index created", helpers.Field{Key: "index_id", Value: resp.Definition.ID})

	if err := c.planInitialShards(ctx, resp.Definition); err != nil {
		c.log.Error("planning initial shards failed", helpers.Field{Key: "error", Value: err}, helpers.Field{Key: "index_id", Value: resp.Definition.ID})
		return coreindex.CreateIndexResponse{}, err
	}

	if err := c.assignPendingShardsToReadyNodes(ctx); err != nil {
		c.log.Error("assign pending shards failed", helpers.Field{Key: "error", Value: err}, helpers.Field{Key: "index_id", Value: resp.Definition.ID})
		return coreindex.CreateIndexResponse{}, err
	}
...
func (c *Coordinator) GetIndex(ctx context.Context, id string) (coreindex.IndexDefinition, error) {
	ctx, span := otel.Tracer("coordinator.index").Start(ctx, "Coordinator.GetIndex")
	defer span.End()

	if c == nil || c.indexRepo == nil {
		return coreindex.IndexDefinition{}, errors.New("coordinator not initialised")
	}

	def, err := c.indexRepo.GetIndex(ctx, id)
	if err != nil {
		if errors.Is(err, coreindex.ErrIndexNotFound) {
			c.log.Info("index lookup returned not found", helpers.Field{Key: "index_id", Value: id})
		} else {
			c.log.Error("get index failed", helpers.Field{Key: "error", Value: err}, helpers.Field{Key: "index_id", Value: id})
		}
		return coreindex.IndexDefinition{}, err
	}
		...
		for {
			...
			case <-ticker.C:
				if err := c.markStaleNodes(timeout); err != nil {
					c.Logger().Error("health monitor error", helpers.Field{Key: "error", Value: err})
				}
			}
		}
	}()
...
func (c *Coordinator) markStaleNodes(timeout time.Duration) error {
	...
	if affected > 0 {
		c.Logger().Info("marked nodes unreachable", helpers.Field{Key: "count", Value: affected})
	}
