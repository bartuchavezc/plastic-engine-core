...
func AssignShardsToNodeTx(ctx context.Context, tx *sql.Tx, nodeID string, limit int) ([]searchshard.Assignment, error) {
	...
	assignments := make([]searchshard.Assignment, 0)
...
func buildAssignment(shardID, indexID, shardKey, analyzer, tokenizer string, mappingVersion int) searchshard.Assignment {
	return searchshard.Assignment{
		ID:               shardID,
		IndexID:          indexID,
		ShardKey:         shardKey,
		Primary:          true,
