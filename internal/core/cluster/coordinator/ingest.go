...
import (
	...
	"plastic-engine-core/internal/core/cluster/coordinator/shards"
...
// Request represents an ingestion command accepted by the coordinator.
type Request struct {
	IndexID    string
	DocumentID string
...
type IndexRepository interface {
	GetIndex(ctx context.Context, id string) (coreindex.IndexDefinition, error)
}
...
func (h *Handler) Handle(ctx context.Context, req Request) error {
	ctx, span := otel.Tracer("coordinator.ingest").Start(ctx, "Coordinator.IngestDocument")
	defer span.End()
	...
	if h.Logger == nil {
		h.Logger = helpers.DefaultLogger()
	}
	...
	h.Logger.Info("document forwarded",
		...
		helpers.Field{Key: "shard_id", Value: shard.ID},
		helpers.Field{Key: "node_id", Value: nodeInfo.ID},
	)
