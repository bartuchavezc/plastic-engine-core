...
// Client implements the node.ClusterClient interface using HTTP calls to the coordinator.
type Client struct {
	baseURL    string
...
func (c *Client) Join(ctx context.Context, info node.NodeInfo) (node.JoinResponse, error) {
	...
	payload := coordinator.JoinRequest{
		NodeID:        info.ID,
		Role:          info.Role,
	...
	var response coordinator.JoinResponse
...
// Heartbeat sends the current node status to the coordinator.
func (c *Client) Heartbeat(ctx context.Context, report node.HeartbeatReport) error {
	...
	payload := coordinator.HeartbeatRequest{
