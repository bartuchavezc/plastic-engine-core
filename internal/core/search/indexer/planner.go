...
func (p *FieldPlanner) BuildPlans(def coreindex.IndexDefinition) ([]FieldPlan, error) {
	...
	for _, mapping := range def.FieldMappings {
		...
		case coreindex.FieldTypeKeyword, coreindex.FieldTypeInteger:
			plans = append(plans, FieldPlan{
				Field: mapping,
			})
