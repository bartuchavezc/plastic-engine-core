...
type WriterFactory func(shard *searchshard.Shard) *IndexWriter

type Service struct {
	shards        *searchshard.Manager
...
func NewService(shards *searchshard.Manager, assignments *AssignmentProvider, planner *FieldPlanner, writerFactory WriterFactory, cfg ShardWorkerConfig, log helpers.Logger) *Service {
	if log == nil {
		log = helpers.DefaultLogger()
	}
	return &Service{
		shards:        shards,
