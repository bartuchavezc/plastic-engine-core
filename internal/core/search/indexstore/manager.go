...
// GetPositionsForDoc performs a point lookup for positions of a single doc+term.
func (m *Manager) GetPositionsForDoc(field, term, docID string) ([]int, error) {
	return m.store.GetPositionsForDoc(field, term, docID)
}
...
// GetAvgDocLen returns the average document length (unique terms) for a field.
func (m *Manager) GetAvgDocLen(field string) float64 {
	return m.store.GetAvgDocLen(field)
}
